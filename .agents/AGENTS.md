...
- **Asterisk Configuration & Sync**:
  ...
  - **WebRTC SIP.js Contact Rewriting Rule**: PJSIP WebRTC endpoints MUST set `rewrite_contact=no`. If set to `yes`, Asterisk rewrites the `Contact` header in `SIP/2.0 200 OK` to the server's public IP, causing SIP.js (`No Contact header pointing to us`) to reject `200 OK` and abort registration. In SIP.js options, `contactURI: uri` must be explicitly defined so SIP.js sends the exact domain URI in the Contact header instead of RFC 5737 dummy IPs (`192.0.2.x`).
