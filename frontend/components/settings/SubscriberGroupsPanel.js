...
export default function SubscriberGroupsPanel({ backendHost }) {
  ...
  const fetchData = async () => {
    try {
      const [resGroups, resUsers] = await Promise.all([
        fetch(`${API_BASE}/api/settings/subscriber_groups`),
  ...
  const handleOpenModal = (group = null) => {
    ...
    if (group) {
      ...
      setFormData({
        ...
        allowed_outbound_groups: group.allowed_outbound_groups || [],
        allowed_inbound_groups: group.allowed_inbound_groups || []
      });
    } else {
  ...
  const handleSave = async () => {
    ...
    if (dupGroup) {
      setError(`'${groupName}' isimli bir abone grubu zaten mevcut. Lütfen farklı bir isim giriniz.`);
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/settings/subscriber_groups`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setGroups(data.subscriber_groups || data);
        setSuccess(true);
  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(`${protocol}//${apiHost}/api/settings/subscriber_groups/${deleteConfirm.id}`, {
        method: "DELETE"
      });
      if (res.ok) {
        const data = await res.json();
        setGroups(data.subscriber_groups);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-8 bg-slate-50/30 dark:bg-slate-950/20">
              ...
              {/* Aranabilecek Gruplar */}
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <DualListBox 
                  title="Aranabilecek Gruplar (Kimi Arayabilir?)"
                  availableItems={groups.filter(g => !selectedGroup || g.id !== selectedGroup.id)}
                  selectedIds={formData.allowed_outbound_groups}
                  onChangeSelectedIds={(newIds) => setFormData(prev => ({...prev, allowed_outbound_groups: newIds}))}
                  columns={["Grup Adı"]}
                  bg={bg} hover={hover} text={text} border={border} lightBg={lightBg}
                  renderRow={(item) => (
              ...
              {/* Çağrı Alınacak Gruplar */}
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <DualListBox 
                  title="Çağrı Alınacak Gruplar (Kimden Çağrı Alabilir?)"
                  availableItems={groups.filter(g => !selectedGroup || g.id !== selectedGroup.id)}
                  selectedIds={formData.allowed_inbound_groups}
                  onChangeSelectedIds={(newIds) => setFormData(prev => ({...prev, allowed_inbound_groups: newIds}))}
                  columns={["Grup Adı"]}
                  bg={bg} hover={hover} text={text} border={border} lightBg={lightBg}
                  renderRow={(item) => (
                    <td className="p-2 text-slate-800 dark:text-slate-200 font-medium">{item.name || "-"}</td>
